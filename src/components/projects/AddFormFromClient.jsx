...
const AddFormFromClient = (props) => {
  ...
  const getMetaData = async () => {
    try {
      const response = await GetRequestHelper("clientlist", navigate);
      if (response.status === 404) {
        setClientData([]);
      } else {
        const updatedData = response.clients.map(client => {
          return {
            client_id: client.id,
...
export default AddFormFromClient;